...
from core.utils.extractor import MetadataExtractorFactory
from core.utils.validator import ValidatorFactory


@click.command()
@click.option("--file", required=True, type=str, help="本地素材文件的绝对路径")
def main(file):
    """本地素材文件上传工具，上传素材并获取媒资ID"""
    ...
    try:
        ...
        # 创建元数据提取器和校验器
        extractor = MetadataExtractorFactory()
        validator = ValidatorFactory()

        # 上传素材文件
        click.echo(f"正在上传素材文件: {file}")
