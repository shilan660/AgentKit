...
def delete_file_from_kb(file_id: int) -> dict:
    """
    删除知识库文件
    ...
    url = build_kb_api_url(BYTEHOUSE_HOST, 'knowledge-base/file/delete')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='ByteHouse 知识库删除内容工具',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
示例:
  # 删除指定文件
  python3 delete_content_to_kb.py --file-id 123
...
'''
    ...
    parser.add_argument('--file-id', type=int, required=True, help='指定要删除的文件ID（必填）')
    
    args = parser.parse_args()
    
    try:
        file_id = args.file_id
        result = delete_file_from_kb(file_id)
