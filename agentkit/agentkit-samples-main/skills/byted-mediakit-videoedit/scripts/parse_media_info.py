...
def parse_bilibili_json_subtitle(json_path: str) -> list[dict]:
    ...
    for item in body:
        ...
        if text:
            subtitles.append(
                {"time": time, "end_time": end_time, "type": "subtitle", "text": text}
            )
    return sorted(subtitles, key=lambda x: x["time"])
...
def find_density_peaks(
    timeline: list[dict], top_n: int = 10, min_gap: float = 10.0
) -> list[dict]:
    """找到弹幕密度最高的区间（高能时刻候选）。
    ...
    if not danmaku_items:
        return []
    sorted_items = sorted(
        danmaku_items, key=lambda x: x.get("density", 0), reverse=True
    )
    peaks = []
...
示例（多视频）:
  %(prog)s \\
    ...
    parser.add_argument(
        ...
        dest="danmakus",
        required=True,
        help="Bilibili XML 弹幕文件路径，可重复传入多次，与 --video 按顺序对应",
    )
    parser.add_argument(
        "--subtitle",
        action="append",
    ...
    danmakus = args.danmakus
    # 未传 --video 时用弹幕路径的 stem 作为占位标识
    videos = args.videos if args.videos else [Path(d).stem for d in danmakus]
    subtitles = args.subtitles + [None] * (len(danmakus) - len(args.subtitles))

    if len(videos) != len(danmakus):
        print(
            f"[错误] --video 数量({len(videos)})与 --danmaku 数量({len(danmakus)})不一致"
        )
        raise SystemExit(1)
    ...
    for idx, (video_file, danmaku_path, subtitle_path) in enumerate(
        zip(videos, danmakus, subtitles), 1
    ):
        prefix = f"[视频{idx}/{len(danmakus)}]"
        print(f"{prefix} 视频: {video_file}")

        # 始终解析弹幕用于统计，是否写入输出由下方规则决定
        ...
        if write_danmaku:
            ...
            print(
                f"  → 弹幕按 {args.interval}s 区间汇总，生成 {len(danmaku_summaries)} 个摘要，写入输出"
            )
        else:
            danmaku_summaries = []
            print(
                "  → 已有字幕，弹幕摘要不写入输出（使用 --include-danmaku 可强制包含）"
            )
        ...
        video_timeline = sorted(
            danmaku_summaries + subtitle_list, key=lambda x: x["time"]
        )
    ...
    print(
        f"  共 {len(videos)} 个视频，{total_danmaku} 条弹幕，{total_subtitles} 条字幕"
    )
    if global_peaks:
        peak_summary = [
            (p.get("video_file", ""), f"{p['time_start']:.0f}s")
            for p in global_peaks[:5]
        ]
