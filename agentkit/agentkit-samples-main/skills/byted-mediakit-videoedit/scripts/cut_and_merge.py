...
def normalize_clip_resolution(
    clip_path: str, output_path: str, target_w: int, target_h: int
) -> bool:
...
def merge_clips_with_transition(
    clip_paths: list[str], transition: str, transition_duration: float, output_path: str
) -> bool:
...
def main():
    parser = argparse.ArgumentParser(
        description="根据 clips JSON 使用 FFmpeg 剪辑合并视频"
    )
    parser.add_argument(
        "--clips-json", required=True, help="clips JSON 文件路径（或 JSON 字符串）"
    )
    ...
    output_path = (
        ...
        or config.get("output", "/tmp/output_cut.mp4")
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if len(unique_res) > 1:
                ...
                for i, cp in enumerate(clip_files):
                    ...
                    if (w, h) == (target_w, target_h):
                        ...
                        print(
                            f"  [{i + 1}/{len(clip_files)}] {w}x{h} → {target_w}x{target_h}（补黑边）"
                        )
        ...
        if transition == "none" or len(clip_files) == 1:
            ...
            success = merge_clips_with_transition(
                clip_files, transition, transition_duration, output_path
            )
