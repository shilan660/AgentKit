...
from common import DEFAULT_ENV_PATH, build_billing_api, build_request_from_official_kwargs, print_response, validate_date_in_month, validate_pagination


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="分页查询账单明细")
    ...
    parser.add_argument("--GroupTerm", "--group-term", dest="group_term", type=int, choices=[0, 1, 2, 3], default=argparse.SUPPRESS, help="统计项")
...
def main() -> None:
    ...
    request = build_request_from_official_kwargs(
        volcenginesdkbilling.ListBillDetailRequest,
        {
            ...
            "GroupTerm": getattr(args, "group_term", None),
    ...
    try:
        response = api_instance.list_bill_detail(request)
        print_response(response)
    except ApiException as exc:
        raise SystemExit(f"ListBillDetail 调用失败: {exc}")
