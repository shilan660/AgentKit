...
"""
火山引擎 RDS PostgreSQL 运维助手调用脚本
用于接收用户命令并调用火山引擎 RDS PostgreSQL API，返回结果
"""
...
try:
    ...
    from volcenginesdkrdspostgresql import models
    ...
    print(
        "错误: 缺少必要的依赖包。请先安装: pip install 'volcengine-python-sdk[rdspostgresqlv2,vpc]'",
        file=sys.stderr,
    )
...
class RDSPostgreSQLClient:
    """火山引擎 RDS PostgreSQL 客户端封装"""

    def __init__(self, region: str = "cn-beijing", endpoint: Optional[str] = None):
        """
        初始化 RDS PostgreSQL 客户端
    ...
    def _create_client(self) -> RDSPOSTGRESQLApi:
        """创建火山引擎 RDS PostgreSQL 客户端"""
    ...
    def list_instances(
        ...
        if db_engine_version:
            req_params["db_engine_version"] = db_engine_version
    ...
    def list_parameter_templates(
        ...
        if template_category:
            req_params["template_category"] = template_category
    ...
    def get_price(
        self,
        node_spec: str = "rds.postgres.1c2g",
        ...
        charge_info = {"ChargeType": charge_type}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="火山引擎 RDS PostgreSQL 运维助手命令行工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 查询实例列表
  python call_rds_postgresql.py list-instances

  # 查询指定实例详情
  python call_rds_postgresql.py describe-instance --instance-id postgres-xxx

  # 查询实例的数据库列表
  python call_rds_postgresql.py list-databases --instance-id postgres-xxx

  # 查询实例的账号列表
  python call_rds_postgresql.py list-accounts --instance-id postgres-xxx

  # 查询实例参数
  python call_rds_postgresql.py list-parameters --instance-id postgres-xxx

  # 查询 VPC 列表
  python call_rds_postgresql.py list-vpcs

  # 查询子网列表
  python call_rds_postgresql.py list-subnets --vpc-id vpc-xxx --zone-id cn-beijing-a
        """,
    )
    ...
    list_instances_parser = subparsers.add_parser(
        "list-instances", help="查询 RDS PostgreSQL 实例列表"
    )
    ...
    list_instances_parser.add_argument(
        "--db-engine-version",
        dest="db_engine_version",
        help="数据库引擎版本（可选，如 PostgreSQL_13）",
    )
    ...
    list_templates_parser.add_argument(
        ...
        help="数据库版本（如 PostgreSQL_13、PostgreSQL_14）",
    )
    list_templates_parser.add_argument(
        "--template-source",
    ...
    get_price_parser.add_argument(
        ...
        help="节点规格（默认: rds.postgres.1c2g）",
    )
    get_price_parser.add_argument(
        "--storage-space",
        dest="storage_space",
    ...
    try:
        client = RDSPostgreSQLClient(region=args.region, endpoint=args.endpoint)
        ...
        if args.action == "list-instances":
            result = client.list_instances(
                ...
                db_engine_version=getattr(args, "db_engine_version", None),
            )
            ...
            result = client.list_parameter_templates(
                ...
                template_category=getattr(args, "template_category", None),
            )
        ...
        print(f"\n调用 RDS PostgreSQL API 时出错: {e}", file=sys.stderr)
        sys.exit(1)
