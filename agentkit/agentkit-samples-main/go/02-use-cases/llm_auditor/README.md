# LLM 审核助手 - LLM Auditor (Go)

## 概述

这是一个基于火山引擎 VeADK-Go 构建的智能审核助手，旨在作为自动化的事实核查助手，评估并增强大语言模型 (LLM) 生成内容的真实性。 

本助手通过识别文本中的可验证主张，利用 AgentKit 平台提供的 `web_search` 工具进行联网搜索和核查，生成详细的审核报告，并可选地修正原始回答中的不准确之处。

## 核心功能

- **识别事实主张**：自动提取 LLM 生成文本中具体、可验证的陈述。
- **验证主张准确性**：利用联网搜索工具 (`web_search`) 验证主张的真伪。
- **生成审核报告**：输出清晰的核查报告，列出每个主张的验证状态（准确、不准确、有争议等）。
