...
# VikingDB 知识库集合名（必需）
# 首次运行会自动创建同名 Collection；后续运行会复用该 Collection
export DATABASE_VIKING_COLLECTION=agentkit_knowledge_app

# TOS 对象存储 Bucket（必需）
# 本地文档先上传到 TOS，再由 VikingDB 从 TOS 拉取进行向量化入库
export DATABASE_TOS_BUCKET=<Your TOS Bucket Name>

# （可选）TOS Bucket 所在区域与 Endpoint，默认 cn-hongkong
# 国内常用可以改为 cn-beijing
# export DATABASE_TOS_REGION=cn-beijing
# export DATABASE_TOS_ENDPOINT=tos-cn-beijing.volces.com

# （可选）输出语言：设为 byteplus 后使用英文 prompt
# export CLOUD_PROVIDER=byteplus
```

> 上述变量也可以写到项目目录下的 `.env` 文件，启动时 VeADK 会自动加载。注意将 `.env` 加入 `.gitignore`，避免凭证被提交。
...
# 新开一个终端，运行测试客户端（本地调用默认 base_url=http://127.0.0.1:8000，
# 无需修改 client.py。如需调用云端，再按后面「AgentKit 云上部署」一节的说明修改）
uv run client.py
```
